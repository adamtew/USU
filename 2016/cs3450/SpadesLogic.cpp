...
void Spades::score()
{
	for (auto p : players)
	{
		...
		auto bag = p->getBags();
		if (bid == tricks) // if you get the same number of tricks you bid for then you just multiply the bid by 10 and that' the score for that round
		{
			p->setRoundScore(bid * 10);
		}
		else if (bid < tricks) // if it's les than you get 10 times the bid + 1 for every extra trick you got
		{
			int sc = bid * 10;
		...
		else if (bid > tricks) // if the bid is greater than you have to count up the bags and  
		{
      if (bag > 10) {
        p->setRoundScore(-100);
        p->setBags(bag % 10);
